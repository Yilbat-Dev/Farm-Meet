...
class ProduceSerializer(serializers.HyperlinkedModelSerializer):
    ...
    class Meta:
        model = FarmProduce
        fields = ['id','name', 'farmer_profile', 'description', 'produce_categories','images', 'created_at', 'produce_status', 'price',
                   'pickup_location']
...
class BankAccountSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = BankAccount
        fields = "__all__"
        read_only_fields = ["farmer", "paystack_recipient_code", "created_at"]

class WithdrawalRequestSerializer(serializers.ModelSerializer):
    farmer = FarmerProfileSerializer()
    class Meta:
        model = WithdrawalRequest
        fields = ["id", "farmer", "amount", "status", "created_at", "reference"]
        read_only_fields = ["id", "status", "created_at", "reference"]